...
class ClusterSuperSystem(supersystem.SuperSystem):
    ...
    def get_supersystem_energy(self):
        ...
        if self.ct_energy is None:
            ...
            #temp_pipek = pipek.PM(self.mol).kernel(self.ct_scf.mo_coeff)
            #temp_boys = boys.Boys(self.mol).kernel(self.ct_scf.mo_coeff)

            #N = np.array([0.0 for i in range(self.mol.nao_nr())])
            #for i in range(int(self.mol.tot_electrons()/2)):
            #    N[i] = 2.
            #temp_pipek_dmat = np.dot((temp_pipek * N), temp_pipek.transpose().conjugate())
            #temp_boys_dmat = np.dot((temp_boys * N), temp_boys.transpose().conjugate())

            #print("TEMPPIPEK")
            #print("TEMPPIPEKA")
            #temp_zeros_a = np.zeros_like(temp_pipek_dmat)
            #temp_zeros_b = np.zeros_like(temp_pipek_dmat)
            #temp_zeros_a[:self.subsystems[0].mol.nao_nr(), :self.subsystems[0].mol.nao_nr()] = np.copy(temp_pipek_dmat[:self.subsystems[0].mol.nao_nr(), :self.subsystems[0].mol.nao_nr()])
            #temp_zeros_b[self.subsystems[0].mol.nao_nr():, self.subsystems[0].mol.nao_nr():] = np.copy(temp_pipek_dmat[self.subsystems[0].mol.nao_nr():, self.subsystems[0].mol.nao_nr():])
            #print (np.trace(np.dot(temp_pipek_dmat, temp_smat)))
            #print (temp_zeros_a)
            #print (temp_zeros_b)
            #print (np.trace(np.dot(temp_zeros_a, temp_smat)))
            #print("TEMPPIPEKB")
            #print (np.trace(np.dot(temp_zeros_b, temp_smat)))

            #print("TEMPBoys")
            #print("TEMPBoysA")
            #temp_zeros_a = np.zeros_like(temp_pipek_dmat)
            #temp_zeros_b = np.zeros_like(temp_pipek_dmat)
            #temp_zeros_a[:self.subsystems[0].mol.nao_nr(), :self.subsystems[0].mol.nao_nr()] = np.copy(temp_boys_dmat[:self.subsystems[0].mol.nao_nr(), :self.subsystems[0].mol.nao_nr()])
            #temp_zeros_b[self.subsystems[0].mol.nao_nr():, self.subsystems[0].mol.nao_nr():] = np.copy(temp_boys_dmat[self.subsystems[0].mol.nao_nr():, self.subsystems[0].mol.nao_nr():])
            #print (np.trace(np.dot(temp_boys_dmat, temp_smat)))
            #print (temp_zeros_a)
            #print (temp_zeros_b)
            #print (np.trace(np.dot(temp_zeros_a, temp_smat)))
            #print("TEMPBOYSB")
